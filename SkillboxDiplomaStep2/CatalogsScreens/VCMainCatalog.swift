...
class VCMainCatalog: UIViewController {
  ...
  //   if let viewController = segue.destination as? VCProduct, segue.identifier == "segueToVCCatalogGoods" {
  //   }
  // }
...
extension VCMainCatalog {
  ...
  func makeDataSourceCategoriesMode() -> DataSourceAliastCatalogMode {
    ...
    let categoriesArray = CatalogData.instance.getCategoriesArray()
    let dataSource = DataSourceAliastCatalogMode(collectionView: catalogCollectionView, cellProvider: { collectionView, indexPath, _ -> UICollectionViewCell? in
